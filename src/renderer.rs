...
impl Renderer {
    ...
    pub async fn new(
        ...
        match Self::init_wgpu(window, window_size, &config).await {
            Ok((device, queue, surface, surface_config, render_pipeline)) => {
    ...
    fn render_wgpu(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        // Generate vertices from canvas strokes
    ...
    /// Render using Cairo (for fallback)
    pub fn render_cairo(&self, cr: &gtk4::cairo::Context, width: i32, height: i32) {
        // Clear background
        cr.set_source_rgb(
            self.config.clear_color.r as f64 / 255.0,
            self.config.clear_color.g as f64 / 255.0,
            self.config.clear_color.b as f64 / 255.0,
        );
        cr.paint().unwrap();
